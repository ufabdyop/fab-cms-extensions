...
class Page_router extends CI_Controller {
	...
	*/
	function _remap_cms()
	{
		...
		$output = '';
		$use_cache = $this->config->item('use_page_cache', 'fuel') ;
		...
		if ($use_cache !== 'false' AND $this->config->item('use_page_cache', 'fuel') !== 'views' AND $this->cache->get($cache_id, $cache_group, FALSE) AND (is_true_val($this->fuel_page->is_cached)) AND !is_fuelified())
		{
